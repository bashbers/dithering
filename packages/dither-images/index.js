...
var foundPngFiles = recursiveFindByExtension(__dirname, 'png', null, []);
console.log(foundPngFiles);
var _loop_1 = function (i) {
    var foundPng = foundPngFiles[i];
    ...
    fs.createReadStream(foundPng)
        ...
        var palette = [[25, 25, 25], [75, 75, 75], [125, 125, 125], [175, 175, 175], [225, 225, 225], [250, 250, 250]];
        var dithered = BayerDithering(pixels, this.width, this.height, palette);
        // The final image
        var png = new PNG({
            width: this.width,
            height: this.height,
        ...
        for (var i_1 = 0; i_1 < dithered.length; i_1++) {
            var color = palette[dithered[i_1]]; // [R, G, B]
            var rgbaOffset = i_1 * 4;
        ...
        for (var i_2 = 0; i_2 < rgba.length; i_2++) {
            png.data[i_2] = rgba[i_2] || 0;
        }
        //Optimize range is between 1 and 7. Its lossless so we use 7.
        var optimizer = new OptiPng(['-o7']);
        var ditheredFilename = foundPng.split('.')[0] + "-dithered.png";
        png.pack()
            .pipe(optimizer)
            .pipe(fs.createWriteStream(ditheredFilename)).on('finish', function () {
            console.log('Success!');
        });
    });
};
for (var i = 0; i < foundPngFiles.length; i++) {
    _loop_1(i);
}
// console.log(foundPngFiles);
